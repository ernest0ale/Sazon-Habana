...
function initMapGeneral() {
  ...
  centerControl.onAdd = function() {
    ...
    div.onclick = function() { leafletMap.setView([23.1136, -82.3666], 12); };
    return div;
  };
...
function createMapPopupMarker(rest) {
  ...
  const resenas = getResenasByRestauranteId(rest.id);
  const promedio = resenas.length ? (resenas.reduce((a,b)=>a+b.puntuacion,0)/resenas.length).toFixed(1) : "Nuevo";
...
function resetMapFilters() {
  ...
  updateMapMarkers();
  if(leafletMap) leafletMap.setView([23.1136, -82.3666], 12);
}